...
float vrotateX = 0, vrotateY = 0, vrotateZ = 0;  // rotation amounts about axes, controlled by keyboard
...
void mykeyboard(unsigned char key, int x, int y)
{
	...
	case 'd': //move camera to the left 
	...
	case 'w': // move camera close (same direction)
		cameraPos = cameraPos + alfa * cameraFront;
		break;

	case 'z': // move camera close (same direction)
		cameraPos = cameraPos + alfa * cameraUp;
		break;

	case 'x': // move camera close (same direction)
		cameraPos = cameraPos - alfa * cameraUp;
		break;
...
void drawScene(void)
{
	...
	View = rotate(View, radians(vrotateX), cameraUp);
	View = rotate(View, radians(vrotateY), cross(cameraUp, cameraFront));
